...
public class Mandala
{
   ...
   public static float[] PredictionValidation = { 1.0f, -1.0f, 1.0f, -1.0f, 1.0f, -1.0f, 1.0f, -1.0f };
   ...
   public static void main(String[] args)
   {
      ...
      for (int i = 0; i < args.length; i++)
      {
         ...
         if (args[i].equals("-numCausationHierarchies"))
         {
            ...
            if (NUM_CAUSATION_HIERARCHIES < 1)
            {
               System.err.println("Invalid numCausationHierarchies option");
               System.err.println(Usage);
               System.exit(1);
      ...
      if (causationPaths == null)
      {
         ...
         for (int i = 0; i < NUM_CAUSATION_HIERARCHIES; i++)
         {
            ...
            for (int j = 0; j < NUM_CAUSATION_PATHS; j++)
            {
               ...
               int k = causationHierarchy.size();
               if ((j > 0) && (k > 1))
               {
                  while (root.id == 0)
                  {
                     root = causationHierarchy.get(randomizer.nextInt(k));
                  }
               }
   ...
   public static void exportNNdataset(String filename, float trainFraction, int randomSeed)
   {
            ...
            for (int k = 0, p = path.steps.size() - 1; k < p; k++)
            {
               ...
               for (int t = 0; t < maxTiers; t++)
               {
                  if (t == 0)
                  {
                     ...
                     {
                        y_train_step.add(PredictionValidation[q]);
                     }
                     for (int q = 0; q < NUM_DIMENSIONS; q++)
                     {
      ...
      for (int i = (int)((float)NUM_CAUSATION_PATHS * NN_DATASET_TRAIN_FRACTION); i < NUM_CAUSATION_PATHS; i++)
      {
         ...
         while (true)
         {
            ...
            if (VERBOSE)
            {
               ...
               System.out.println("path=" + i + ", hierarchy=" + h + ", step=" + s + "+" + (s + 1));
            }
            ...
            if (xstep.size() > 1)
            {
               ...
               if (causationTier.currentChild == 0)
               {
      ...
      try
      {
         ...
         printWriter.println("]");
         printWriter.println("y_train_shape = [ " + y_train.size() + ", " + (PredictionValidation.length + (maxTiers * NUM_DIMENSIONS)) + " ]");
         ...
         printWriter.println("]");
         printWriter.println("y_test_shape = [ " + y_test.size() + ", " + (PredictionValidation.length + (maxTiers * NUM_DIMENSIONS)) + " ]");
   ...
   public static void exportRNNdataset(String filename, float trainFraction, int randomSeed)
   {
      ...
      int    numPaths = NUM_CAUSATION_HIERARCHIES * NUM_CAUSATION_PATHS;
      ArrayList < ArrayList < Float >> X_train = new ArrayList < ArrayList < Float >> ();
      ArrayList < ArrayList < Float >> y_train = new ArrayList < ArrayList < Float >> ();
      ...
      {
         ArrayList<CausationPath> paths = causationPaths.get(i);
         for (int j = (int)((float)NUM_CAUSATION_PATHS * NN_DATASET_TRAIN_FRACTION); j < NUM_CAUSATION_PATHS; j++)
         {
            CausationPath path     = paths.get(j);
            int           sequence = 0;
            if (VERBOSE)
